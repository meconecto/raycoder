...
export class FakeAgentAdapter implements AgentAdapter {
  ...
  public async *send(session: AgentSession, prompt: string): AsyncIterable<AgentEvent> {
    ...
    yield { type: "assistant_message", timestamp: timestamp(), text: fakeAssistantMessage(state, prompt) };
...
function fakeAssistantMessage(state: FakeSessionState, prompt: string): string {
  if (state.purpose !== "planning") return `Fake turn ${state.turn + 1}: ${prompt}`;
  if (prompt.includes("Planning stage: spec.")) {
    return JSON.stringify({
      title: "Deterministic specification",
      summary: "A deterministic specification generated by the fake adapter.",
      goals: ["Exercise the planning pipeline"],
      nonGoals: ["Contact a real provider"],
      requirements: ["Persist the approved planning context"],
      acceptanceCriteria: ["The generated artifact is valid structured data"],
      constraints: ["Remain offline and deterministic"],
    });
  }
  if (prompt.includes("Planning stage: tickets.")) {
    return JSON.stringify({
      tickets: [
        { id: "plan-core", title: "Build core", description: "Implement the planned core.", predecessorIds: [] },
        { id: "plan-ui", title: "Build UI", description: "Expose the planned core in the UI.", predecessorIds: ["plan-core"] },
      ],
    });
  }
  return `Fake turn ${state.turn + 1}: ${prompt}`;
}
